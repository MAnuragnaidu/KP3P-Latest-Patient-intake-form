...
export function isNoneExclusiveOptionDisabled(
  opt: string,
  selected: string[],
  noneOption: string = NONE_EXCLUSIVE_OPTION,
): boolean {
  const noneSelected = selected.includes(noneOption);
  const hasOtherSelections = selected.some((item) => item !== noneOption);
  return (noneSelected && opt !== noneOption) || (hasOtherSelections && opt === noneOption);
}
