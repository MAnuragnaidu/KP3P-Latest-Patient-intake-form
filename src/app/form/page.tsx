...
export default function FormPage() {
  ...
  const nextStep = async () => {
    ...
    if (step === 1) {
      ...
      try {
        const duplicate = await checkDuplicatePatient(data.email, data.contactPhone);
        if (!duplicate.ok) {
          if (duplicate.field === 'email' || duplicate.field === 'contactPhone') {
            setErrors({ [duplicate.field]: duplicate.error });
          } else {
            setErrors({ submit: duplicate.error });
          }
          window.scrollTo({ top: 0, behavior: 'smooth' });
          return;
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409 && (resData.field === 'email' || resData.field === 'contactPhone')) {
          setErrors({ [resData.field]: resData.error || 'This patient already exists.' });
          setStep(1);
          window.scrollTo({ top: 0, behavior: 'smooth' });
          return;
        }
      ...
      <main className="page-main">
        ...
        <div className="step-card pb-36 md:pb-0" key={step}>
          ...
          <div className={`form-nav fixed bottom-0 left-0 right-0 z-50 bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-[0_-4px_10px_rgba(0,0,0,0.08)] md:relative md:p-0 md:bg-transparent md:shadow-none md:z-auto ${step < TOTAL_STEPS ? 'form-nav-mobile-hide' : ''}`}>
            ...
            <div className="form-nav-right">
              {step < TOTAL_STEPS ? (
                <button type="button" className="btn-next" onClick={nextStep} disabled={isCheckingDuplicate}>
        ...
        {step < TOTAL_STEPS && (
          <div className="step-minimap">
            ...
            {step < TOTAL_STEPS && (
              <button type="button" className="minimap-btn minimap-btn-next" onClick={nextStep} disabled={isCheckingDuplicate}>
