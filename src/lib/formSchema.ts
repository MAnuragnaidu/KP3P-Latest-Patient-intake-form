...
/** Upper bound for DOB date picker (today, local timezone). */
...
export function isValidDateOfBirthIso(value: string): boolean {
...
export function isValidDateOfBirth(value: string): boolean {
  return isValidDateOfBirthIso(value) || isValidDateOfBirthDdMmYyyy(value);
}
...
export function parseDateOfBirth(value: string): Date | null {
  if (isValidDateOfBirthIso(value)) {
    const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value.trim())!;
    const year = Number(match[1]);
    const month = Number(match[2]);
    const day = Number(match[3]);
  ...
  return parseDateOfBirthDdMmYyyy(value);
}
...
export function validateStep(step: number, d: FormData): Record<string, string> {
  ...
  if (step === 1) {
    ...
    if (d.dateOfBirth && !isValidDateOfBirth(d.dateOfBirth))
      err['dateOfBirth'] = 'Please select a valid date of birth';
